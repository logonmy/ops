...
IPv6ranges = {
    ...
    '0010000000000001':
    'ASSIGNABLE',  # 2001::/16 Sub-TLA Assignments [RFC2450]
    '00100000000000010000000':
    'ASSIGNABLE IANA',  # 2001:0000::/29 - 2001:01F8::/29 IANA
    '00100000000000010000001':
    'ASSIGNABLE APNIC',  # 2001:0200::/29 - 2001:03F8::/29 APNIC
    '00100000000000010000010':
    'ASSIGNABLE ARIN',  # 2001:0400::/29 - 2001:05F8::/29 ARIN
    '00100000000000010000011':
    'ASSIGNABLE RIPE',  # 2001:0600::/29 - 2001:07F8::/29 RIPE NCC
...
class IPint:
    ...
    def __init__(self, data, ipversion=0, make_net=0):
        ...
        # handle IP instance as an parameter
            ...
            if make_net:
                self.ip = self.ip & _prefixlenToNetmask(self._prefixlen,
                                                        self._ipversion)

            if not _checkNetaddrWorksWithPrefixlen(self.ip, self._prefixlen,
                                                   self._ipversion):
    ...
    def strCompressed(self, wantprefixlen=None):
        ...
        if self._ipversion == 4:
            ...
            if max(followingzeros) > 1:
                ...
                while compressionpos < len(hextets) and hextets[
                        compressionpos] == '0':
    ...
    def strFullsize(self, wantprefixlen=None):
        ...
        return intToIp(
            self.ip,
            self._ipversion).lower() + self._printPrefix(wantprefixlen)
    ...
    def __contains__(self, item):
        ...
        item = IP(item)
        if item.ip >= self.ip and item.ip < self.ip + self.len() - item.len(
        ) + 1:
            return 1
        else:
            return 0
...
def _parseAddressIPv6(ipstr):
    ...
    if fill_pos is not None:
        diff = 8 - len(items)
        if diff <= 0:
            raise ValueError("%r: Invalid IPv6 address: '::' is not needed" %
                             ipstr)
    ...
    if len(items) != 8:
        ...
        raise ValueError("%r: Invalid IPv6 address: should have 8 hextets" %
                         ipstr)
    ...
    for item in items:
        try:
            ...
            error = True
        if error:
            raise ValueError("%r: Invalid IPv6 address: invalid hexlet %r" %
                             (ipstr, item))
        value = (value << 16) + item
        index += 1
    return value
...
def parseAddress(ipstr):
    ...
    if ipstr.startswith('0x'):
        ...
        if ret > 0xffffffffffffffffffffffffffffffffL:
            raise ValueError, "%r: IP Address can't be bigger than 2^128" % (
                ipstr)
        if ret < 0x100000000L:
            return (ret, 4)
        else:
    ...
    if ipstr.find(':') != -1:
        ...
        for x in bytes:
            if x > 255 or x < 0:
                raise ValueError, "%r: single byte must be 0 <= byte < 256" % (
                    ipstr)
...
def intToIp(ip, version):
    ...
    if version == 4:
        if ip > 0xffffffffL:
            raise ValueError, "IPv4 Addresses can't be larger than 0xffffffff: %s" % (
                hex(ip))
        for l in range(4):
        ...
        if ip > 0xffffffffffffffffffffffffffffffffL:
            raise ValueError, "IPv6 Addresses can't be larger than 0xffffffffffffffffffffffffffffffff: %s" % (
                hex(ip))
        l = '0' * 32 + hex(ip)[2:-1]
        for x in range(1, 33):
            ret = l[-x] + ret
...
def _prefixlenToNetmask(prefixlen, version):
    ...
    return (
        (2L << prefixlen - 1) - 1) << (_ipVersionToLen(version) - prefixlen)
