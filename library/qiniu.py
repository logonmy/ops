...
def signing(secret_key, data):
    return urlsafe_b64encode(hmac.new(secret_key, data, hashlib.sha1).digest())
...
def requests_error_handler(func):
    ...
    def deco(*args, **kwargs):
        try:
            ...
            req = e.args[0]
            raise QiniuException(req.url, req.status_code, req.content)
...
class UploadToken(object):
    ...
    def _make_token(self):
        self.generated_at = int(time.time())
        info = {'scope': self.scope, 'deadline': self.generated_at + self.ttl}
...
class Qiniu(object):
    ...
    def put(self, scope, filename, data=None, keep_name=False, override=True):
        ...
        files = {'file': data}
        action = '/rs-put/%s' % urlsafe_b64encode('%s:%s' %
                                                  (scope, upload_name))
    ...
    def _stat_rm_handler(self, action, bucket, filename):
        url = '%s/%s/%s' % (RS_HOST, action,
                            urlsafe_b64encode('%s:%s' % (bucket, filename)))
        return self.api_call(url)


class Bucket(object):
    def __init__(self, qiniu, bucket):
        self.qiniu = qiniu
        self.bucket = bucket

    def put(self, filename, data=None, keep_name=False, override=True):
        return self.qiniu.put(self.bucket,
                              filename,
                              data=data,
                              keep_name=keep_name,
                              override=override)
    ...
    def stat(self, filename):
        return self.qiniu.stat(self.bucket, filename)

    def delete(self, filename):
        return self.qiniu.delete(self.bucket, filename)

    def copy(self, *args):
        return self.qiniu.copy(self._build_cp_mv_args(*args))
