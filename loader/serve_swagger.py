...
class SpecServer():
    ...
    def process_inbound_request(self):
        ...
        if self.handle_preflight_request():
            return  # it would be good if we could just force a response return
        ...
        try:
            for k in swagger_spec['paths'].keys():
                for http_method in swagger_spec['paths'][k].keys():
                    uri_fields, uri_template = compile_uri_template(
                        '/' + http_method.lower() + '/3.0' + k)
                    self.routing_templates.append(repr(uri_template))
                    operationId = swagger_spec['paths'][k][http_method][
                        'operationId']
                    self.routing_table[operationId] = {'uri_fields':
                                                       uri_fields,
                                                       'uri_template':
                                                       uri_template}
    ...
    def handle_preflight_request(self):
        ...
        if self.req.method == 'OPTIONS':
            ...
            self.resp.set_header('Access-Control-Allow-Methods',
                                 'GET, POST, PUT, PATCH, DELETE, OPTIONS')
            self.resp.set_header(
                'Access-Control-Allow-Headers',
                'X-Auth-Token, Keep-Alive, Users-Agent, X-Requested-With, If-Modified-Since, Cache-Control, Content-Type')
    ...
    def match_request_url_to_operation(self):
        ...
        for operation, v in sorted(self.routing_table.items(), reverse=False):
            ...
            route_signature = '/' + self.req.method.lower(
            ) + self.req.relative_uri
            log.info("Trying to match route signature: {}".format(
                route_signature))
            m = v['uri_template'].match(route_signature)
            if m:
                log.info("Request URL matches route signature: {}".format(
                    route_signature))
    ...
    def dispatch_matched_operation_to_request_handlers(self):
        ...
        matched_operation_handlers = self.operation_handlers[
        ...
        for item in matched_operation_handlers:
            ...
            log.info("Sending request to operation_request_handler: {}".format(
                item))
