...
class connection:
    ...
    def get_error(self):
        if self.debug:
            self.logger.debug("reporting error: {}".format(self.__e))
        #if self.__e is None: return False
        return self.__errors()

    def __exit(self):
        return dict(results=self.__results, errors=self.__errors)
...
# ------------------------------------------------------------------------
    ...
    def __nc_connect(self):
        ...
        try:
            if self.private_key is not None:
                ...
                self.nc = netconf_ssh.connect(host=self.target,
    ...
    def __nc_close(self):
        if self.debug:
            self.logger.debug("closing netconf session: {}".format(
                self.target))
    ...
    def __ssh_connect(self):
        ...
        try:
            if self.private_key is not None and os.path.isfile(
                    self.private_key):
