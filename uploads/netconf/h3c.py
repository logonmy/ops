...
class H3CNetConfDriver():
    ...
    def _create_tunnel_xml(self, tunnel_id, src, dst):
        return self.create_tunnel_xml.format(tunnel_id=tunnel_id,
                                             src_addr=src,
                                             dst_addr=dst)
    ...
    def _create_port_ac_xml(self, index, service_id, vsiname):
        return self.create_ac_xml.format(if_index=index,
    ...
    def _create_port_ac(self, index, service_id, vsiname):
        return self._operation(self._create_port_ac_xml(index, service_id,
                                                        vsiname))
    ...
    def _create_service_xml(self, index, service_id, s_vid):
        return self.create_service_xml.format(if_index=index,
                                              service_id=service_id,
                                              s_vid=s_vid)

    def _delete_service_xml(self, index, service_id):
        return self.delete_service_xml.format(if_index=index,
                                              service_id=service_id)

    def _create_service(self, index, service_id, s_vid):
        return self._operation(self._create_service_xml(index, service_id,
                                                        s_vid))
    ...
    def newvlan2vxlan(self, index, vlan, vxlan):
        ...
        print "newvlan2vxlan: vxlan %s vlan %s index %s tor %s" % (
            vxlan, vlan, index, self.mgr)
