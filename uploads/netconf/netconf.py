...
class RPCClient(object):
    def __init__(self, device, username='', password=''):
        ...
        try:
            ...
            raise Exception(
                "Specified device ({}) does not have a primary IP defined.".format(
                    device))
    ...
    def __enter__(self):
        ...
        try:
            ...
            try:
                ...
                if default_creds.get('username') and default_creds.get(
                        'password'):
                    self.ssh.connect(self.host,
...
class JunosNC(RPCClient):
    ...
    def get_lldp_neighbors(self):
        ...
        lldp_neighbors_raw = xmltodict.parse(rpc_reply.xml)['rpc-reply'][
            'lldp-neighbors-information']['lldp-neighbor-information']

        result = []
        for neighbor_raw in lldp_neighbors_raw:
            neighbor = dict()
            neighbor['local-interface'] = neighbor_raw.get(
                'lldp-local-port-id')
            neighbor['name'] = neighbor_raw.get('lldp-remote-system-name')
            neighbor['name'] = neighbor['name'].split('.')[
                0]  # Split hostname from domain if one is present
            try:
                neighbor['remote-interface'] = neighbor_raw[
                    'lldp-remote-port-description']
            except KeyError:
                # Older versions of Junos report on interface ID instead of description
                neighbor['remote-interface'] = neighbor_raw.get(
                    'lldp-remote-port-id')
    ...
    def get_inventory(self):
        def glean_modules(node, depth=0):
            ...
            for module in modules_list:
                m = {
                    'name': module['name'],
                    'part_id': module.get('model-number') or
        ...
        inventory_raw = xmltodict.parse(rpc_reply.xml)['rpc-reply'][
        ...
        def modules(chassis_serial=None):
            ...
            for i in cmd:
                ...
                try:
                    ...
                    if m_serial != chassis_serial and m_pid.lower(
                    ) != 'unspecified':
                        yield {
                            'name': m_name,
                            'part_id': m_pid,
